<?php
// Même logique de récupération des filtres que nouveautes.php, mais uniquement pour la catégorie 'femme'
$host = 'localhost';
$dbname = 'stylish';
$username = 'root';
$password = '';
...
try {
    ...
    // Récupérer les types uniques pour femme
    $stmt_types = $pdo->query("SELECT DISTINCT type FROM produit WHERE catégorie = 'femme' ORDER BY type");
    $types = $stmt_types->fetchAll(PDO::FETCH_COLUMN);

    // Récupérer les couleurs uniques pour femme
    $stmt_colors = $pdo->query("SELECT DISTINCT couleur FROM produit WHERE catégorie = 'femme' ORDER BY couleur");
    $colors = $stmt_colors->fetchAll(PDO::FETCH_COLUMN);

    // Récupérer les marques uniques pour femme
    $stmt_brands = $pdo->query("SELECT DISTINCT marque FROM produit WHERE catégorie = 'femme' ORDER BY marque");
    $brands = $stmt_brands->fetchAll(PDO::FETCH_COLUMN);

    // Récupérer les pointures disponibles pour femme (jointure sur pointure_produit)
    $stmt_sizes = $pdo->query("SELECT DISTINCT p.pointure FROM pointures p JOIN pointure_produit pp ON p.id = pp.id_pointure JOIN produit pr ON pr.id = pp.id_produit WHERE pr.catégorie = 'femme' ORDER BY p.pointure");
...
<section id="promotions-products" class="product-store py-2 my-2 py-md-5 my-md-5 pt-0">
  <div class="container-md">
    <div class="display-header d-flex align-items-center justify-content-center">
      <h2 class="section-title-center text-uppercase">Liste des chaussures femmes</h2>
    </div>
    <div class="row">
      <div class="col-md-3">
...
<script>
  ...
  function loadFilteredProducts() {
      ...
      fetch(`get_filtered_femmes.php?${params.toString()}`)
