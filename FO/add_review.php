...
try {
    ...
    $stmt = $pdo->prepare("SELECT id FROM avis WHERE id_utilisateur = ? AND id_produit = ?");
    ...
    // Ajouter l'avis
    $stmt = $pdo->prepare("INSERT INTO avis (id_utilisateur, id_produit, note, commentaire, date_creation) 
                          VALUES (?, ?, ?, ?, NOW())");
